...
def reports():
    ...
    selected_product_id = None    # ✅ to remember what you selected
    ...
    if request.method == 'POST':
        ...
        if report_type == 'lifecycle':
            ...
            # ---------------------------------
            # ✅ FIX: Use callproc for stored procedures
            # ---------------------------------
            cursor.callproc('GetLifecycleReport', (inst_id,))
            # Iterate through stored results to get data AND clear the connection
            for result in cursor.stored_results():
    ...
    # This query now runs safely after the connection is cleared
    cursor.execute("""
        SELECT 
            c.ComponentID,
            c.ComponentName,
